...
class SecDrainage(PDrainage):
    ...
    def popUpdateOilInj(self):
        ...
        try:
            ...
            ar2 = arr[(self.fluid[arr]==1) & (self.trappedNW[arr])]
    ...
    def untrapNWElements(self, ind):
        ...
        arr = []
        for i in idx:
            arr.extend(self.elementLists[(self.trapCluster_NW==i)[1:-1]])
        arr = np.array(arr)
        ...
        newElements = []
        try:
            newElements.extend([*chain(*self.PTConData[arrP]+self.nPores)])
        except IndexError:
            pass
        try:
            newElements.extend([*chain(
                self.P1array[arrT-self.nPores-1], self.P2array[arrT-self.nPores-1])])
        except IndexError:
            pass
        newElements = np.array(newElements)
        newElements = newElements[(self.fluid[newElements]==0)]
        self.__update_PcD_ToFill__(newElements)
